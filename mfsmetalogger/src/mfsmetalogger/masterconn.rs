...
pub unsafe extern "C" fn masterconn_createpacket(
    ...
    if outpacket.is_null() {
        fprintf(
            ...
            b"/tmp/moosefs-ref/mfsmetalogger/masterconn.c\0".as_ptr() as *const ::core::ffi::c_char,
...
pub unsafe extern "C" fn masterconn_download_next(mut eptr: *mut masterconn) {
    ...
    if (*eptr).dloffset >= (*eptr).filesize {
        ...
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_INFO,
            b"%s downloaded %luB/%lu.%06us (%.3lf MB/s)\0".as_ptr() as *const ::core::ffi::c_char,
        ...
        ptr = masterconn_createpacket(eptr, ANTOMA_DOWNLOAD_REQUEST as uint32_t, 12 as uint32_t);
...
pub unsafe extern "C" fn masterconn_download_info(
    ...
    if length != 1 as uint32_t && length != 8 as uint32_t {
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_WARNING,
            b"MATOAN_DOWNLOAD_INFO - wrong size (%u/1|8)\0".as_ptr() as *const ::core::ffi::c_char,
            length,
        );
        (*eptr).mode = KILL as ::core::ffi::c_int as uint8_t;
        return;
    ...
    if (*eptr).masteraddrvalid as ::core::ffi::c_int == 0 as ::core::ffi::c_int {
        ...
        ) >= 0 as ::core::ffi::c_int
        {
            ...
            mfs_log(
                MFSLOG_SYSLOG_STDERR,
                MFSLOG_WARNING,
                b"can't resolve master host/port (%s:%s)\0".as_ptr() as *const ::core::ffi::c_char,
                MasterHost,
                MasterPort,
            );
            return -1 as ::core::ffi::c_int;
