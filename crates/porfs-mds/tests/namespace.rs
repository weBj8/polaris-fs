...
fn create_lookup_getattr() {
    ...
    let f = mds.create(ROOT_INO, "file", 0o644).unwrap();
...
fn create_existing_name_fails() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    mds.create(ROOT_INO, "dup", 0o644).unwrap();
    let err = mds.create(ROOT_INO, "dup", 0o644).unwrap_err();
    assert!(matches!(err, MdsError::Exists(_)), "{err:?}");
    assert!(matches!(
        mds.mkdir(ROOT_INO, "dup", 0o755).unwrap_err(),
        MdsError::Exists(_)
    ));
}
...
fn mkdir_nested_and_readdir_order() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    let a = mds.mkdir(ROOT_INO, "a", 0o755).unwrap();
    let b = mds.mkdir(a, "b", 0o700).unwrap();
    let f = mds.create(b, "f", 0o600).unwrap();
    mds.create(ROOT_INO, "z", 0o644).unwrap();
    assert_eq!(mds.lookup(a, "b").unwrap(), b);
    assert_eq!(mds.lookup(b, "f").unwrap(), f);
    assert_eq!(mds.getattr(b).unwrap().mode, 0o700);
    // readdir comes back in byte (name) order from the B-tree.
    let root_entries = mds.readdir(ROOT_INO).unwrap();
    assert_eq!(names(&root_entries), ["a", "z"]);
    assert_eq!(root_entries[0].1, a);
    assert_eq!(root_entries[0].2, NodeKind::Dir);
    assert_eq!(root_entries[1].2, NodeKind::File);
    assert_eq!(names(&mds.readdir(b).unwrap()), ["f"]);
}
...
fn dir_ops_on_files_fail() {
    ...
    assert!(matches!(
        mds.create(f, "x", 0o644).unwrap_err(),
        MdsError::NotDir(_)
    ));
}
...
fn unlink_dir_fails() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    mds.mkdir(ROOT_INO, "d", 0o755).unwrap();
    assert!(matches!(
...
fn rmdir_rules() {
    ...
    mds.create(d, "child", 0o644).unwrap();
    assert!(matches!(
...
fn hard_links_share_inode_and_data() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    let f = mds.create(ROOT_INO, "orig", 0o644).unwrap();
...
fn rename_same_dir_keeps_inode() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    let f = mds.create(ROOT_INO, "before", 0o644).unwrap();
...
fn rename_overwrites_existing_file() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    let a = mds.create(ROOT_INO, "a", 0o644).unwrap();
    mds.write(a, 0, b"winner").unwrap();
    let b = mds.create(ROOT_INO, "b", 0o644).unwrap();
...
fn rename_dir_over_empty_dir() {
    ...
    let mds = &mut format_mds(dir.path());
    let a = mds.mkdir(ROOT_INO, "a", 0o755).unwrap();
    mds.create(a, "child", 0o644).unwrap();
    let b = mds.mkdir(ROOT_INO, "b", 0o755).unwrap();
    mds.rename(ROOT_INO, "a", ROOT_INO, "b").unwrap();
    assert_eq!(mds.lookup(ROOT_INO, "b").unwrap(), a);
    assert!(matches!(mds.getattr(b).unwrap_err(), MdsError::NotFound(_)));
    assert_eq!(names(&mds.readdir(a).unwrap()), ["child"]);
...
fn rename_dir_over_nonempty_dir_fails() {
    let dir = test_dir();
    let mds = &mut format_mds(dir.path());
    mds.mkdir(ROOT_INO, "a", 0o755).unwrap();
    let b = mds.mkdir(ROOT_INO, "b", 0o755).unwrap();
    mds.create(b, "occupied", 0o644).unwrap();
    assert!(matches!(
        mds.rename(ROOT_INO, "a", ROOT_INO, "b").unwrap_err(),
        MdsError::NotEmpty(_)
    ));
    // Nothing changed.
    assert_eq!(names(&mds.readdir(ROOT_INO).unwrap()), ["a", "b"]);
}
...
fn rename_onto_same_name_is_noop() {
    ...
    assert_eq!(mds.getattr(f).unwrap().nlink, 2);
    assert_eq!(names(&mds.readdir(ROOT_INO).unwrap()), ["f", "g"]);
}
...
fn invalid_names_rejected() {
    ...
    for bad in ["", "a/b", ".", "..", long.as_str()] {
        assert!(
            matches!(
                mds.create(ROOT_INO, bad, 0o644),
                Err(MdsError::InvalidName(_))
            ),
            "create accepted {bad:?}"
        );
        assert!(matches!(
            mds.mkdir(ROOT_INO, bad, 0o755),
            Err(MdsError::InvalidName(_))
        ));
        assert!(matches!(
            mds.link(f, ROOT_INO, bad),
            Err(MdsError::InvalidName(_))
        ));
        assert!(matches!(
            mds.lookup(ROOT_INO, bad),
            Err(MdsError::InvalidName(_))
        ));
        assert!(matches!(
            mds.rename(ROOT_INO, "f", ROOT_INO, bad),
            Err(MdsError::InvalidName(_))
        ));
        assert!(matches!(
            mds.unlink(ROOT_INO, bad),
            Err(MdsError::InvalidName(_))
        ));
    }
    // A 255-byte name is legal.
    let max = "y".repeat(255);
    mds.create(ROOT_INO, &max, 0o644).unwrap();
...
fn reopen_preserves_namespace() {
    ...
    let mut mds = format_mds(dir.path());
    let d = mds.mkdir(ROOT_INO, "d", 0o755).unwrap();
    let f = mds.create(d, "f", 0o640).unwrap();
    mds.link(f, ROOT_INO, "alias").unwrap();
    drop(mds);
    ...
    assert_eq!(names(&mds.readdir(ROOT_INO).unwrap()), ["alias", "d"]);
    mds.self_check().unwrap();
}
