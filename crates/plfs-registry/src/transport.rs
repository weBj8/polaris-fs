//! Registry raft peer transport: openraft AppendEntries / Vote /
//! InstallSnapshot over gRPC (`plfs.meta.v1.MetaRaft` envelope), bincode
//! payloads — the same stack as plfs-meta::transport (design doc §2),
//! specialized for [`RegistryRaftConfig`].
...
use crate::raft::RegistryRaftConfig;
...
impl RaftTransportSvc {
    /// Transport service for this registry node.
    pub fn new(raft: Raft<RegistryRaftConfig>) -> Self {
        Self { raft }
    }
...
impl MetaRaft for RaftTransportSvc {
    async fn append_entries(
        ...
        let rpc: AppendEntriesRequest<RegistryRaftConfig> = dec(&request.into_inner().payload)
    ...
    async fn install_snapshot(
        ...
        let rpc: InstallSnapshotRequest<RegistryRaftConfig> = dec(&request.into_inner().payload)
            .map_err(|e| tonic::Status::invalid_argument(e.to_string()))?;
...
impl RaftNetworkFactory<RegistryRaftConfig> for GrpcNetworkFactory {
...
impl RaftNetwork<RegistryRaftConfig> for GrpcNetworkConnection {
    async fn append_entries(
        &mut self,
