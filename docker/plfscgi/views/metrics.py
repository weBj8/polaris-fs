...
class MFSMetrics:
	...
	# Calls 'plfscli' with appropriate (scope, mastercharts, cscharts) parameters and generates Prometheus metrics restricted by prefix_whitelist and prefix_blacklist
	@classmethod
	...
	def generate_metrics(self, masterhost, masterport, scope, mastercharts, cscharts):
		try:
			...
			# proc = subprocess.Popen(["plfscli", "-j", "-SIN", "-SCS", "-SHD", "-SQU", "-SMC", "-SMO", "-SMS", "-a", "1", "-b", "all", "-SCC", "-c", "1", "-d", "all"],env=clz.make_env(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			json_string = ""
			for line in proc.stdout:
				json_string+=line.decode('utf-8')
			execution_time_seconds = time.time() - start_time
			print('# HELP mfs_cli_execution_time CLI (plfscli) command execution time [seconds] used to fetch metrics')
