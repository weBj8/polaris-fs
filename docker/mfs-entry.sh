...
#
# Common env:
#   ROLE            mfsmaster | mfsmetalogger | mfschunkserver | ...
#   DATA_PATH       daemon working dir inside the container (default /var/lib/mfs)
...
ROLE="${ROLE:-mfsmaster}"
DATA_PATH="${DATA_PATH:-/var/lib/mfs}"
mkdir -p "$DATA_PATH"
...
mfsmount)
    ...
    mkdir -p "$MOUNT_POINT"
    exec mfsmount -f -o allow_other -H "$MASTER_HOST" ${MOUNT_OPTS:-} "$MOUNT_POINT"
    ;;
*)
