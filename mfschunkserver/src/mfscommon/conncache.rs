...
pub unsafe extern "C" fn conncache_init(mut cap: uint32_t) -> ::core::ffi::c_int {
    ...
    while p < capacity {
        ...
        (*conncachetab.offset(p as isize)).hashnext = (if p.wrapping_add(1 as uint32_t) < capacity {
            conncachetab.offset(p.wrapping_add(1 as uint32_t) as isize)
        } else {
            ::core::ptr::null_mut::<connentry>()
        }) as *mut _connentry;
        (*conncachetab.offset(p as isize)).hashprev = ::core::ptr::null_mut::<*mut _connentry>();
        p = p.wrapping_add(1);
    }
