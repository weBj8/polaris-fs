...
pub unsafe extern "C" fn charts_seriescnt(mut r#type: uint32_t) -> uint8_t {
    ...
    {
        s = 0 as uint8_t;
        src = (*estatdefs.offset(r#type.wrapping_sub(CHARTS_EXTENDED_START as uint32_t) as isize))
            .c1src;
        ...
        && r#type < (CHARTS_EXTENDED_START as uint32_t).wrapping_add(estatdefscount)
    {
            ...
            && src < (CHARTS_CALC_START as uint32_t).wrapping_add(calcdefscount)
        {
            ...
            while i < width {
                ...
                sp = 0 as uint32_t;
                ops =
                    *calcstartpos.offset(src.wrapping_sub(CHARTS_CALC_START as uint32_t) as isize);
                while *ops != CHARTS_OP_END as uint32_t {
                    if *ops < statdefscount {
                        if sp < 50 as uint32_t {
                            if *(*series.offset(*ops as isize))[range as usize].offset(j as isize)
...
pub unsafe extern "C" fn charts_add(mut data: *mut uint64_t, mut datats: uint32_t) {
    ...
    if delta <= 0 as int32_t && delta > -(MAXLENG as int32_t) && !data.is_null() {
        ...
        while j < statdefscount {
            ...
            } else if (*statdefs.offset(j as isize)).mode as ::core::ffi::c_int == CHARTS_MODE_ADD {
                *(*series.offset(j as isize))[SHORTRANGE as usize].offset(i as isize) =
                    (*(*series.offset(j as isize))[SHORTRANGE as usize].offset(i as isize))
...
pub unsafe extern "C" fn charts_init(
    ...
    if i > 0 as uint32_t && calcdefscount > 0 as uint32_t {
        calcdefs =
            malloc(::core::mem::size_of::<uint32_t>().wrapping_mul(i as size_t)) as *mut uint32_t;
        if calcdefs.is_null() {
            fprintf(
                stderr,
                b"%s:%u - out of memory: %s is NULL\n\0".as_ptr() as *const ::core::ffi::c_char,
        ...
        calcstartpos =
            malloc(::core::mem::size_of::<*mut uint32_t>().wrapping_mul(calcdefscount as size_t))
                as *mut *mut uint32_t;
        if calcstartpos.is_null() {
            fprintf(
                stderr,
    ...
    if statdefscount > 0 as uint32_t {
        statdefs = malloc(::core::mem::size_of::<statdef>().wrapping_mul(statdefscount as size_t))
            as *mut statdef;
        if statdefs.is_null() {
            fprintf(
                stderr,
    ...
    while i < estatdefscount {
        if !(*estats.offset(i as isize)).name.is_null() {
            ...
            (*estatdefs.offset(i as isize)).name = ::core::ptr::null_mut::<::core::ffi::c_char>();
        }
    ...
    if statdefscount > 0 as uint32_t {
        monotonic = malloc(::core::mem::size_of::<uint64_t>().wrapping_mul(statdefscount as size_t))
            as *mut uint64_t;
        if monotonic.is_null() {
            fprintf(
                stderr,
        ...
        series = malloc(::core::mem::size_of::<stat_record>().wrapping_mul(statdefscount as size_t))
            as *mut stat_record;
        if series.is_null() {
            fprintf(
                stderr,
        ...
        while i < statdefscount {
            *monotonic.offset(i as isize) = 0 as uint64_t;
            j = 0 as uint32_t;
            while j < RANGES as uint32_t {
                (*series.offset(i as isize))[j as usize] =
                    malloc((MAXLENG as size_t).wrapping_mul(::core::mem::size_of::<uint64_t>()))
                        as *mut uint64_t;
                if (*series.offset(i as isize))[j as usize].is_null() {
                    fprintf(
                        stderr,
        ...
        while fy < 9 as uint32_t {
            ...
            if fbits != 0 {
                ...
                while fx < 5 as uint32_t {
                    ...
                    fbits = ((fbits as ::core::ffi::c_int) << 1 as ::core::ffi::c_int) as uint8_t;
...
pub unsafe extern "C" fn charts_makechart(
    ...
    if r#type < statdefscount {
        ...
        && r#type < (CHARTS_EXTENDED_START as uint32_t).wrapping_add(estatdefscount)
    {
        dmax *= (*estatdefs.offset(r#type.wrapping_sub(CHARTS_EXTENDED_START as uint32_t) as isize))
                ...
                == CHARTS_MODE_ADD
    {
        match range {
            ...
            3 => {
                dmax *=
                    (24 as ::core::ffi::c_int * 60 as ::core::ffi::c_int) as ::core::ffi::c_double;
            }
            _ => {}
        }
    }
    ...
    while i < width as int32_t {
        ...
        if j == 0 as int32_t {
            ...
            while j < height as int32_t {
                *chart.offset(
                    (MAXXSIZE as int32_t * (j + YPOS as int32_t) + (i + XPOS as int32_t)) as isize,
                    ...
                    *chart.offset(
                        ...
                        + height
                            ...
                            * COLOR_DATA_RANGE as ::core::ffi::c_double
                            / height as ::core::ffi::c_double) as uint8_t;
            ...
            while height as int32_t > j {
                *chart.offset(
                    (MAXXSIZE as int32_t * (j + YPOS as int32_t) + (i + XPOS as int32_t)) as isize,
                ) = ((COLOR_DATA_BEGIN + 2 as ::core::ffi::c_int * COLOR_DATA_RANGE) as uint32_t)
                    .wrapping_add(
                        height
                            .wrapping_sub(1 as uint32_t)
                            .wrapping_sub(j as uint32_t)
    ...
    while i < height as int32_t + 5 as int32_t {
        *chart.offset(
            (MAXXSIZE as uint32_t)
                ...
                .wrapping_add((XPOS - 1 as ::core::ffi::c_int) as uint32_t) as isize,
        ) = COLOR_AXIS as uint8_t;
        *chart.offset(
            (MAXXSIZE as uint32_t)
                .wrapping_mul(
    ...
    while i <= ypts as int32_t {
        ...
        *chart.offset(
            (MAXXSIZE as uint32_t)
                ...
                .wrapping_add((XPOS - 2 as ::core::ffi::c_int) as uint32_t) as isize,
        ) = COLOR_AXIS as uint8_t;
        *chart.offset(
            (MAXXSIZE as uint32_t)
                .wrapping_mul(
                ...
                .wrapping_add((XPOS - 3 as ::core::ffi::c_int) as uint32_t) as isize,
        ) = COLOR_AXIS as uint8_t;
        if i > 0 as int32_t {
            j = 1 as ::core::ffi::c_int as int32_t;
...
pub unsafe extern "C" fn charts_monotonic_data(mut buff: *mut uint8_t) -> uint32_t {
    let mut i: uint32_t = 0;
    if buff.is_null() {
        return ::core::mem::size_of::<uint16_t>()
            .wrapping_add(::core::mem::size_of::<uint64_t>().wrapping_mul(statdefscount as usize))
            as uint32_t;
    }
...
unsafe extern "C" fn charts_statid_converter(
    ...
    if number < 0x1000000 as uint32_t {
        ...
        *chrange = number.wrapping_rem(10 as uint32_t);
        ...
        number = (number as ::core::ffi::c_uint & 0xdfdfdfdf as ::core::ffi::c_uint) as uint32_t;
        i = 0 as uint32_t;
        while i < statdefscount {
...
pub unsafe extern "C" fn charts_data_multiplier(
    ...
    if r#type < statdefscount {
        ...
        && r#type < (CHARTS_EXTENDED_START as uint32_t).wrapping_add(estatdefscount)
    {
        ret *= (*estatdefs.offset(r#type.wrapping_sub(CHARTS_EXTENDED_START as uint32_t) as isize))
            .multiplier as ::core::ffi::c_int as ::core::ffi::c_double;
        ret /= (*estatdefs.offset(r#type.wrapping_sub(CHARTS_EXTENDED_START as uint32_t) as isize))
            .divisor as ::core::ffi::c_int as ::core::ffi::c_double;
        match (*estatdefs.offset(r#type.wrapping_sub(CHARTS_EXTENDED_START as uint32_t) as isize))
...
pub unsafe extern "C" fn charts_get_png(mut buff: *mut uint8_t) {
    ...
    if compsize == 0 as uint32_t {
        ...
        ptr =
            buff.offset(::core::mem::size_of::<[uint8_t; 847]>().wrapping_sub(8 as usize) as isize);
