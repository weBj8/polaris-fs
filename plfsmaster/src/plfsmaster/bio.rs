...
unsafe extern "C" {
    ...
    unsafe fn tcptowait(sock: ::core::ffi::c_int, msectoall: uint32_t) -> ::core::ffi::c_int;
    unsafe fn tcpshutdown(sock: ::core::ffi::c_int);
    ...
    unsafe {
        ...
        if len >= (*b).size as uint64_t {
            ...
            while len.wrapping_sub(ret as uint64_t) > 0x40000000 as uint64_t {
                i = bio_internal_read(b, dst.offset(ret as isize), 0x40000000 as uint32_t)
                    as int64_t;
                ret += i;
            ...
            if ((*b).leng.wrapping_sub((*b).pos) as uint64_t) < len {
                ...
                if ((*b).leng as uint64_t) < len.wrapping_sub(ret as uint64_t) {
                    ...
                    (*b).pos = ((*b).pos as uint64_t)
                        .wrapping_add(len.wrapping_sub(ret as uint64_t))
                        as uint32_t;
                    return len as int64_t;
                }
            } else {
...
pub unsafe extern "C" fn bio_write(
    ...
    unsafe {
        ...
        if len >= (*b).size as uint64_t {
            ...
            ret = 0 as int64_t;
            while len.wrapping_sub(ret as uint64_t) > 0x40000000 as uint64_t {
                i = bio_internal_write(b, src.offset(ret as isize), 0x40000000 as uint32_t)
                    as int64_t;
                if i < 0x40000000 as int64_t {
                    return -1 as int64_t;
                }
