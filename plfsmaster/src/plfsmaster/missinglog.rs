unsafe extern "C" {
    ...
    unsafe fn chunk_remove_from_missing_log(chunkid: uint64_t) -> uint8_t;
}
pub type uint8_t = u8;
...
pub unsafe extern "C" fn missing_log_insert(
    ...
    unsafe {
        ...
        hash = ((inode.wrapping_mul(1363546567 as uint32_t) ^ indx)
            .wrapping_mul(985732289 as uint32_t) as uint64_t
            ^ chunkid) as uint32_t;
        ...
        if buff.is_null() {
            ...
            while i < mloghashprevsize {
                if (*mloghashprev.offset(i as isize)).chunkid != 0 as uint64_t {
                    if chunk_remove_from_missing_log((*mloghashprev.offset(i as isize)).chunkid)
                        != 0
                    {
