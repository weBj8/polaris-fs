...
unsafe extern "C" {
    ...
    unsafe fn hdd_clear_errors(pleng: uint32_t, path: *const uint8_t) -> uint8_t;
    unsafe fn hdd_diskinfo_size() -> uint32_t;
    unsafe fn hdd_diskinfo_data(buff: *mut uint8_t);
    unsafe fn hdd_diskinfo_monotonic_size() -> uint32_t;
    unsafe fn hdd_diskinfo_monotonic_data(buff: *mut uint8_t);
    unsafe fn masterconn_getcsid() -> uint16_t;
    unsafe fn masterconn_getmetaid() -> uint64_t;
    unsafe fn masterconn_getmasterip() -> uint32_t;
    unsafe fn masterconn_getmasterport() -> uint16_t;
    unsafe fn masterconn_forcereconnect();
    ...
    unsafe {
                ...
                as uint32_t
        {
            mfs_log(
                MFSLOG_SYSLOG,
                MFSLOG_WARNING,
                b"ANTOCS_GET_CHUNK_INFO - wrong size (%u/13)\0".as_ptr()
                    as *const ::core::ffi::c_char,
                length,
            );
            (*eptr).state = CLOSE as ::core::ffi::c_int as uint8_t;
    ...
    unsafe {
        ...
        if length < 4 as uint32_t {
            mfs_log(
                MFSLOG_SYSLOG,
                MFSLOG_WARNING,
                b"ANTOCS_CLEAR_ERRORS - wrong size (%u/>=4)\0".as_ptr()
                    as *const ::core::ffi::c_char,
                length,
            );
            (*eptr).state = CLOSE as ::core::ffi::c_int as uint8_t;
        ...
        if length != pleng.wrapping_add(4 as uint32_t) {
            mfs_log(
                MFSLOG_SYSLOG,
                MFSLOG_WARNING,
                b"ANTOCS_CLEAR_ERRORS - wrong size (%u/4+%u)\0".as_ptr()
                    as *const ::core::ffi::c_char,
                length,
                pleng,
            );
    ...
    unsafe {
        ...
        if tcpsetacceptfilter(newlsock) < 0 as ::core::ffi::c_int && *__errno_location() != ENOTSUP
        {
            mfs_log(
                MFSLOG_SYSLOG_STDERR,
                MFSLOG_NOTICE,
                b"main server module: can't set accept filter\0".as_ptr()
                    as *const ::core::ffi::c_char,
            );
        }
        mfs_log(
    ...
    unsafe {
        ...
        if tcpnumlisten(lsock, mylistenip, mylistenport, 100 as uint16_t) < 0 as ::core::ffi::c_int
        {
            mfs_log(
                MFSLOG_ERRNO_SYSLOG_STDERR,
                MFSLOG_ERR,
                b"main server module: can't listen on socket\0".as_ptr()
                    as *const ::core::ffi::c_char,
            );
            return -1 as ::core::ffi::c_int;
        }
