...
unsafe extern "C" fn usage(mut progname: *const ::core::ffi::c_char) {
    unsafe {
        ...
        fprintf(
            fd,
            b"    -o mfscfgfile=CFGFILE       load some mount options from external file (if not specified then use default file: /usr/local/etc/mfs/mfsmount.cfg or /usr/local/etc/mfsmount.cfg)\n\0"
                .as_ptr() as *const ::core::ffi::c_char,
        );
        fprintf(
            fd,
        ...
        if custom_cfg == 0 as ::core::ffi::c_int {
            ...
            if cfgfd < 0 as ::core::ffi::c_int && *__errno_location() == ENOENT {
                ...
                if cfgfd >= 0 as ::core::ffi::c_int {
                    fprintf(
                        stderr,
                        b"default sysconf path has changed - please move mfsmount.cfg from /usr/local/etc/ to /usr/local/etc/mfs/\n\0"
                            .as_ptr() as *const ::core::ffi::c_char,
                    );
                }
            }
