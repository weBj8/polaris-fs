...
pub unsafe extern "C" fn delay_scheduler(
    ...
    loop {
        if exitflag != 0 {
            let mut _mfs_assert_ret_0: ::core::ffi::c_int = pthread_mutex_unlock(&raw mut dlock);
        ...
        if heapelements > 0 as uint32_t {
            ...
            if now < (*heap.offset(0 as isize)).firetime {
                ...
                Some(r#fn.expect("non-null function pointer")).expect("non-null function pointer")(
                    udata,
                );
                let mut _mfs_assert_ret_2: ::core::ffi::c_int = pthread_mutex_lock(&raw mut dlock);
...
pub unsafe extern "C" fn delay_run(
    ...
    (*heap.offset(heapelements as isize)).firetime = monotonic_useconds().wrapping_add(useconds);
...
pub unsafe extern "C" fn delay_init() {
    ...
    waiting = 0 as uint8_t;
    heap = malloc(::core::mem::size_of::<heapelem>().wrapping_mul(1024 as size_t)) as *mut heapelem;
    if heap.is_null() {
        fprintf(
            stderr,
            b"%s:%u - out of memory: %s is NULL\n\0".as_ptr() as *const ::core::ffi::c_char,
    ...
    if _mfs_assert_ret_6 != 0 as ::core::ffi::c_int {
        if _mfs_assert_ret_6 < 0 as ::core::ffi::c_int
            && *__errno_location() != 0 as ::core::ffi::c_int
        {
            let mut _mfs_errorstring_14: *const ::core::ffi::c_char = strerr(*__errno_location());
            mfs_log(
                MFSLOG_SYSLOG,
                MFSLOG_ERR,
                b"%s:%u - unexpected status, '%s' returned: %d (errno=%d: %s)\0".as_ptr()
