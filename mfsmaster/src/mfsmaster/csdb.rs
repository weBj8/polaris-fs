...
unsafe extern "C" {
    ...
    unsafe fn matocsserv_getservdata(
...
pub unsafe extern "C" fn csdb_mr_op(
    ...
    unsafe {
        ...
        match op as ::core::ffi::c_int {
            CSDB_OP_ADD => {
                hash = hash32(
                    ip ^ ((port as ::core::ffi::c_int) << 16 as ::core::ffi::c_int) as uint32_t,
                )
                .wrapping_rem(CSDBHASHSIZE as uint32_t);
                csptr = csdbhash[hash as usize];
                while !csptr.is_null() {
                ...
                if csptr.is_null() {
                    fprintf(
                        ...
                        b"%s:%u - out of memory: %s is NULL\n\0".as_ptr()
                            as *const ::core::ffi::c_char,
                        b"/tmp/moosefs-ref/mfsmaster/csdb.c\0".as_ptr()
                            as *const ::core::ffi::c_char,
        ...
        while !csptr.is_null() {
                ...
                && (*csptr).port as ::core::ffi::c_int == port as ::core::ffi::c_int
            {
                        ...
                        && onoff as ::core::ffi::c_int == 1 as ::core::ffi::c_int
                {
                    ...
                    {
                        (*csptr).maintenance_timeout =
                            main_time().wrapping_add(MaintenanceModeTimeout);
                    } else {
                        (*csptr).maintenance_timeout = 0 as uint32_t;
                    }
                    if onoff != 0 {
