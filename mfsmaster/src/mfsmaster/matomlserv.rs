...
extern "C" {
    ...
    fn changelog_get_minversion() -> uint64_t;
    fn meta_version() -> uint64_t;
    fn meta_do_store_metadata();
...
unsafe extern "C" fn matomlserv_clientname(
    ...
    match (*eptr).clienttype as ::core::ffi::c_int {
        1 => match (*eptr).logstate as ::core::ffi::c_int {
            ...
            2 => return b"METALOGGER-SYNC\0".as_ptr() as *const ::core::ffi::c_char,
    ...
    if ForceTimeout > 0 as uint32_t {
        ...
        while !eptr.is_null() {
                            ...
                            })) as uint32_t)
            {
                ...
                data =
                    matomlserv_create_packet(eptr, ANTOAN_FORCE_TIMEOUT as uint32_t, 2 as uint32_t);
...
pub unsafe extern "C" fn matomlserv_get_config_file(
    ...
    if fdata.is_null() {
        ptr = matomlserv_create_packet(eptr, ANTOAN_CONFIG_FILE_CONTENT as uint32_t, 5 as uint32_t);
        put32bit(&raw mut ptr, msgid);
    ...
    if length < 1 as uint32_t {
        ...
        if rversion as ::core::ffi::c_int == 1 as ::core::ffi::c_int {
                        ...
                        })) as uint32_t
            {
                ...
                p = matomlserv_create_packet(eptr, MATOAN_MASTER_ACK as uint32_t, 5 as uint32_t);
                        ...
                        })) as uint32_t
            {
                ...
                p_0 = matomlserv_create_packet(eptr, MATOAN_MASTER_ACK as uint32_t, 5 as uint32_t);
...
pub unsafe extern "C" fn matomlserv_store_metadata(
    ...
    if length != 0 as uint32_t {
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_WARNING,
            b"ANTOMA_STORE_METADATA - wrong size (%u/0)\0".as_ptr() as *const ::core::ffi::c_char,
            length,
        );
        (*eptr).mode = KILL as ::core::ffi::c_int as uint8_t;
        return;
    ...
    if (*eptr).clienttype as ::core::ffi::c_int != SUPERVISOR as ::core::ffi::c_int {
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_WARNING,
            b"ANTOMA_STORE_METADATA - wrong client type\0".as_ptr() as *const ::core::ffi::c_char,
        );
        (*eptr).mode = KILL as ::core::ffi::c_int as uint8_t;
        return;
    }
...
pub unsafe extern "C" fn matomlserv_download_start(
    ...
    if length != 1 as uint32_t {
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_WARNING,
            b"ANTOMA_DOWNLOAD_START - wrong size (%u/1)\0".as_ptr() as *const ::core::ffi::c_char,
            length,
        );
        (*eptr).mode = KILL as ::core::ffi::c_int as uint8_t;
        return;
    ...
    if (*eptr).upload_meta_fd < 0 as ::core::ffi::c_int {
            ...
            || filenum as ::core::ffi::c_int == 12 as ::core::ffi::c_int
        {
            ...
            ptr = matomlserv_create_packet(eptr, MATOAN_DOWNLOAD_INFO as uint32_t, 1 as uint32_t);
...
pub unsafe extern "C" fn matomlserv_download_request(
    ...
    if (*eptr).upload_meta_fd < 0 as ::core::ffi::c_int {
        mfs_log(
            MFSLOG_SYSLOG,
            MFSLOG_WARNING,
            b"ANTOMA_DOWNLOAD_REQUEST - file not opened\0".as_ptr() as *const ::core::ffi::c_char,
        );
        (*eptr).mode = KILL as ::core::ffi::c_int as uint8_t;
        return;
    }
...
pub unsafe extern "C" fn matomlserv_broadcast_logrotate() {
    ...
    while !eptr.is_null() {
        ...
        {
            data =
                matomlserv_create_packet(eptr, MATOAN_METACHANGES_LOG as uint32_t, 1 as uint32_t);
...
pub unsafe extern "C" fn matomlserv_reload() {
    ...
    mfs_log(
        MFSLOG_SYSLOG_STDERR,
        MFSLOG_INFO,
        b"master <-> metaloggers module: socket address has changed, now listen on %s:%s\0".as_ptr()
            as *const ::core::ffi::c_char,
        ListenHost,
        ListenPort,
    );
...
pub unsafe extern "C" fn matomlserv_init() -> ::core::ffi::c_int {
    ...
    mfs_log(
        MFSLOG_SYSLOG_STDERR,
        MFSLOG_INFO,
        b"master <-> metaloggers module: listen on %s:%s\0".as_ptr() as *const ::core::ffi::c_char,
        ListenHost,
        ListenPort,
    );
    matomlservhead = ::core::ptr::null_mut::<matomlserventry>();
