...
extern "C" {
    ...
    fn flock_file_closed(sessionid: uint32_t, inode: uint32_t);
    fn posix_lock_file_closed(sessionid: uint32_t, inode: uint32_t);
    ...
    if inodecnt > bitmasksize.wrapping_mul(32 as uint32_t) || bitmask.is_null() {
        ...
        if bitmask.is_null() {
            fprintf(
                ...
                b"/tmp/moosefs-ref/mfsmaster/openfiles.c\0".as_ptr() as *const ::core::ffi::c_char,
