...
unsafe extern "C" fn csipmap_parseline(mut str: *const ::core::ffi::c_char) -> uint8_t {
    ...
    loop {
            ...
            && c as ::core::ffi::c_int <= '9' as ::core::ffi::c_int
        {
            ...
            octet = octet
                .wrapping_add((c as ::core::ffi::c_int - '0' as ::core::ffi::c_int) as uint32_t);
    ...
    while getline(&raw mut linebuff, &raw mut lbsize, fd) != -1 as __ssize_t {
        ...
        match status as ::core::ffi::c_int {
            ...
            BAD_ORDER => {
                mfs_log(
                    MFSLOG_SYSLOG_STDERR,
                    MFSLOG_WARNING,
                    b"error parsing ip range in file %s (line %u) - incorrect ip order\0".as_ptr()
                        as *const ::core::ffi::c_char,
                    fname,
                    lno,
                );
        ...
        if current_section as ::core::ffi::c_int >= MAX_SECTIONS {
            mfs_log(
                MFSLOG_SYSLOG_STDERR,
                MFSLOG_WARNING,
                b"error parsing line in file %s (line %u) - too many sections in file\0".as_ptr()
                    as *const ::core::ffi::c_char,
                fname,
                lno,
            );
