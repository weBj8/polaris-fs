...
extern "C" {
    ...
    fn matoclserv_fuse_posix_lock_wake_up(
    ...
    fn of_checknode(sessionid: uint32_t, inode: uint32_t) -> uint8_t;
...
unsafe extern "C" fn posix_lock_find_offensive_lock(
    ...
    while !al.is_null() {
        if (*al).owner != owner || (*al).sessionid != sessionid {
            if posix_lock_test_wlock((*al).ranges, &raw mut r#type, &raw mut start, &raw mut end)
                != 0
            {
                return 1 as ::core::ffi::c_int;
            }
...
unsafe extern "C" fn posix_lock_interrupt(
    ...
    while !wl.is_null() {
        if (*wl).connptr == connptr && (*wl).reqid == reqid {
            matoclserv_fuse_posix_lock_wake_up(connptr, (*wl).msgid, MFS_ERROR_EINTR as uint8_t);
            ...
            if start < lastend {
                if ignoreflag != 0 {
                    mfs_log(
                        MFSLOG_SYSLOG_STDERR,
                        MFSLOG_NOTICE,
                        b"loading posix_locks: lock range not in order !!! (ignoring)\0".as_ptr()
                            as *const ::core::ffi::c_char,
                    );
                    continue;
                } else {
                ...
                && start == lastend
            {
                if ignoreflag != 0 {
                    mfs_log(
                        MFSLOG_SYSLOG_STDERR,
                        MFSLOG_NOTICE,
                        b"loading posix_locks: lock range not connected !!! (ignoring)\0".as_ptr()
                            as *const ::core::ffi::c_char,
                    );
                    continue;
                } else {
                ...
                while !wl.is_null() {
                    fprintf(
                        ...
                        if (*wl).r#type as ::core::ffi::c_int == POSIX_LOCK_RDLCK {
                            'R' as ::core::ffi::c_int
                        } else if (*wl).r#type as ::core::ffi::c_int == POSIX_LOCK_WRLCK
                        {
                            'W' as ::core::ffi::c_int
                        } else {
                            'U' as ::core::ffi::c_int
...
pub unsafe extern "C" fn posix_lock_init() -> ::core::ffi::c_int {
    ...
    inodehash = malloc(
        ::core::mem::size_of::<*mut inodelocks>().wrapping_mul(POSIX_LOCK_INODE_HASHSIZE as size_t),
