...
pub unsafe extern "C" fn merger_start(
    ...
    unsafe {
        ...
        heap =
            malloc(::core::mem::size_of::<hentry>().wrapping_mul(files as size_t)) as *mut hentry;
        if heap.is_null() {
...
pub unsafe extern "C" fn merger_loop(mut verblevel: uint8_t) -> ::core::ffi::c_int {
    unsafe {
        ...
        while heapsize != 0 {
            if (*heap.offset(0 as isize)).nextid % 2497 as int64_t == 0 as int64_t
                && lastlv > firstlv
            {
                if (*heap.offset(0 as isize)).nextid < firstlv as int64_t {
                    ...
                    perc = ((*heap.offset(0 as isize)).nextid as uint64_t)
                        ...
                        .wrapping_div(lastlv.wrapping_sub(firstlv))
                        as uint8_t;
                    eta = lastlv
