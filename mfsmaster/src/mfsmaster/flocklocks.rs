...
extern "C" {
    ...
    fn abort() -> !;
    fn matoclserv_fuse_flock_wake_up(
            ...
            if (*il).active.is_null() && (*il).waiting_head.is_null() {
                ...
                fprintf(
                    ...
                    b"il->active==NULL && il->waiting_head==NULL\0".as_ptr()
                        as *const ::core::ffi::c_char,
                    b"inode flock record not empty !!!\0".as_ptr() as *const ::core::ffi::c_char,
                );
                mfs_log(
                    MFSLOG_SYSLOG,
                    MFSLOG_ERR,
...
unsafe extern "C" fn flock_check(mut il: *mut inodelocks, mut ltype: uint8_t) -> uint8_t {
    if ltype as ::core::ffi::c_int == LTYPE_READER {
        if !(*il).active.is_null() && (*(*il).active).ltype as ::core::ffi::c_int == LTYPE_WRITER {
            return 1 as uint8_t;
        }
