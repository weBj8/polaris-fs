...
pub static mut RunTab: [InitEntry; 2] = [
    InitEntry {
        function: Some(
            plfsgui::src::mfsgui::mfsgui::mfscgiserv_init as unsafe extern "C" fn() -> c_int,
        ),
        name: c"cgi_server".as_ptr() as *mut c_char,
    },
    InitEntry {
        function: None,
...
const APP_NAME: &str = "mfsgui";
const DATA_PATH: &str = "/usr/local/var/mfs";
const DEFAULT_CONFIG: &str = "/usr/local/etc/mfs/mfsgui.cfg";
const FALLBACK_CONFIG: &str = "/usr/local/etc/mfsgui.cfg";
...
fn main() {
    if !daemon_main::configure(daemon_main::DaemonConfig {
        ...
        init: plfsgui::src::mfsgui::mfsgui::mfscgiserv_init,
        init_name: c"cgi_server",
    }) {
        std::process::exit(1);
    }
