...
unsafe fn main_0(
    ...
    unsafe {
        ...
        cfgfile = strdup(
            b"/usr/local/etc/mfs/mfsmetalogger.cfg\0".as_ptr() as *const ::core::ffi::c_char
        );
        if cfgfile.is_null() {
            fprintf(
                stderr,
        ...
        if fd < 0 as ::core::ffi::c_int && *__errno_location() == ENOENT {
            ocfgfile = strdup(
                b"/usr/local/etc/mfsmetalogger.cfg\0".as_ptr() as *const ::core::ffi::c_char
            );
            if ocfgfile.is_null() {
                fprintf(
                    stderr,
        ...
        if movewarning != 0 {
            mfs_log(
                MFSLOG_SYSLOG_STDERR,
                MFSLOG_WARNING,
                b"default sysconf path has changed - please move mfsmetalogger.cfg from /usr/local/etc/ to /usr/local/etc/mfs/\0"
                    .as_ptr() as *const ::core::ffi::c_char,
            );
        }
